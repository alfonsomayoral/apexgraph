"""`.apexgraphignore` matching and node exclusion."""

from __future__ import annotations

from pathlib import Path

from apexgraph.ignore import ApexgraphIgnore, apply_ignore, load_ignore
...
def test_match_by_node_id():
    ignore = ApexgraphIgnore.from_lines(["node:test"])
...
def test_comments_and_blanks_ignored():
    ignore = ApexgraphIgnore.from_lines(["# a comment", "", "   ", "src/util.py"])
...
def test_should_ignore_with_plain_graphify_node_dict():
    ...
    ignore = ApexgraphIgnore.from_lines(["src/main.py"])
...
def test_load_ignore_missing_returns_none(tmp_path: Path):
    assert load_ignore(tmp_path / "nope.apexgraphignore") is None


def test_load_ignore_parses_file(tmp_path: Path):
    p = tmp_path / ".apexgraphignore"
...
def test_apply_ignore_drops_matching_keeps_rest_and_edges():
    g = _graph()
    ignore = ApexgraphIgnore.from_lines(["tests/", "src/util.py"])
